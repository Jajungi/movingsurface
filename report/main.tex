...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}>{\raggedright\arraybackslash}p{3.2cm} >{\raggedright\arraybackslash}p{4.8cm} >{\raggedright\arraybackslash}p{4.6cm}@{}}
        ...
        $S(r)$ & Eq.~\eqref{eq:S_num} & Table~\ref{tab:bounded_setup}; Sec.~\ref{sec:limit_escape} \\
        $L_{z,c}$ & Eq.~\eqref{eq:Lzc_num} & Table~\ref{tab:bounded_setup} \\
        Turning radii & Eqs.~\eqref{eq:turning_analytic}--\eqref{eq:turning_err} & Table~\ref{tab:bounded_orbits} \\
        $\delta_E$ & Eqs.~\eqref{eq:Et_num}--\eqref{eq:deltaE_num} & Figs.~\ref{fig:energy_all}, \ref{fig:dt_scaling}; Tables~\ref{tab:bounded_setup}, \ref{tab:dt_halving} \\
        $\Delta T_r$, $T_a/T_r$, $\langle\Delta\phi\rangle/(2\pi)$ & Eqs.~\eqref{eq:Tr_num}--\eqref{eq:closure_residual} & Table~\ref{tab:bounded_orbits}; Fig.~\ref{fig:xy_precession} \\
...
\begin{equation}\label{eq:S_num}
    S(r) = 3z'(r) + r\,z''(r).
\end{equation}
At a candidate circular radius $r_c$, local stability of the radial EAPEF requires $S(r_c)\ge 0$ when $z'(r_c)>0$ (Eq.~\eqref{eq:stability}). This quantity appears in Table~\ref{tab:bounded_setup} at $r=r_0$.
...
\end{equation}
Reported cone/paraboloid agreements are $\le 0.043\%$ (Table~\ref{tab:bounded_orbits}).
...
Sec.~\ref{sec:results} applies these definitions: Sec.~\ref{sec:bounded} reports the cone and paraboloid results in Tables~\ref{tab:bounded_setup} and~\ref{tab:bounded_orbits}; Sec.~\ref{sec:limit_escape} treats the Gaussian escape runs in Table~\ref{tab:gauss_limit}.
...
The cone $z=r$ and paraboloid $z=r^2$ each have two zeros of $U_{\mathrm{eq}}(r)$, confining the particle between inner and outer turning radii. Tables~\ref{tab:bounded_setup} and~\ref{tab:bounded_orbits} summarize the initial conditions, energy monitoring, turning-point agreement, and periodic radial dynamics ($m=g=1$, $\Delta t=0.005\,\mathrm{s}$).

\begin{table}[htbp]
    \centering
    \caption{Bounded-motion setup and energy monitoring: cone and paraboloid ($m=g=1$, $\Delta t=0.005\,\mathrm{s}$).}
    \label{tab:bounded_setup}
    \small
...
\begin{table}[htbp]
    \centering
    \caption{Turning points and periodic radial dynamics: cone and paraboloid ($m=g=1$, $\Delta t=0.005\,\mathrm{s}$).}
    \label{tab:bounded_orbits}
    \small
    \renewcommand{\arraystretch}{1.22}
    \setlength{\tabcolsep}{8pt}
    \sisetup{table-format=1.4}
...
Explicit Euler integration yielded turning-point agreement at the 0.04\% level on the cone and paraboloid, period ratios $T_a/T_r=1.244$ and $0.536$, azimuthal precession $\langle\Delta\phi\rangle/(2\pi)=0.804$ and $1.864$ per radial cycle (Fig.~\ref{fig:xy_precession}), and energy drifts below $0.44\%$ over 100 s (Tables~\ref{tab:bounded_setup} and~\ref{tab:bounded_orbits}, Fig.~\ref{fig:dt_scaling}).
