...
\author{
    \begin{tabular}{r @{\quad} l}
        ...
        \textbf{Department:} & Semiconductor Engineering, DGIST
    \end{tabular}
}
\date{\today}
...
Numerical results in Secs.~\ref{sec:numerical}--\ref{sec:results} were obtained with the explicit Euler scheme in Sec.~\ref{sec:simulation} ($\Delta t=0.005\,\mathrm{s}$, $m=g=1$).
...
The figures below were generated from the Euler integration in Sec.~\ref{sec:simulation}. Each time series was subsampled to at most 320 points per curve for plotting.
